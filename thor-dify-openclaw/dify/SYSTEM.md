...
3. **第一趟快馬**：沒有說飛、沒有說認錯 → `scout=none`、`verify=none`（驛馬直接去，可能夾錯）。
4. **第二趟加持**：說「此非荔枝／先探／認清楚」→ `scout=crazyflie`、`verify=yolo`。飛鴿＋燈塔定位，狗到點後 YOLO 確認才夾。
...
- `[助理=reachy] 把那盒嶺南荔枝送到沙發給貴妃`
  → `{"intent":"fetch","item":"嶺南荔枝","dest":"沙發","recipient":"貴妃","scout":"none","verify":"none","confirm":true,"say":"驛馬快去，將荔枝送至貴妃案前"}`
- `[助理=reachy] 此非荔枝。派飛鴿探路，到點用 YOLO 確認`
  → `{"intent":"fetch","item":"嶺南荔枝","dest":"沙發","recipient":"貴妃","scout":"crazyflie","verify":"yolo","confirm":true,"say":"飛鴿先探枝頭，驛馬到點，火眼認荔而後取"}`
